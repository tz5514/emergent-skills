...
4. **Designators.** Each design question's named options are A, B, C, … restarting every question. The user answers this turn with the letter, and refers across questions by writing the number themselves (`2B` or `1 B, 2 A`). Option lines carry only the letter.
5. **Batch boundary.** Whichever of 🧭 / ✅ a cycle actually emits is emitted once per documentation-gate cycle: when one cycle writes several files or triggers several checks, the results merge into a single block of each kind.
...
7. **Block order within a gate cycle is fixed:** 🧭 → 📝 → ✅.

## Ask markers — ❓ and ✋

The turn's ask is a `### ` heading, so the interviewee can scan it the same way as the other blocks. SKILL.md states which ask gets which heading. At most one ❓ or one ✋ heading per output turn, so that heading keeps meaning "this is the ask".

Like the other block-heading emoji, ❓ and ✋ are copied unchanged in every environment language; only the surrounding prose is translated.
...
```
### ❓ Question N: {full question}
{optional supplement, may be empty}

- A: …
- B: … (recommended: {reason})
```

Order is fixed: heading → supplement → named-option list. An empty supplement means the list follows the heading directly. Named options appear only in that list — not in the heading, not in the question sentence, not only inside a table/tree/flow/diff. The recommendation mark sits on the recommended option and includes the reason. Tables, trees, sequences, and diffs may fill the supplement; they never replace the list.

### Other ruling points

Two fixed ✋ titles. Neither is numbered as 問題 / Question N. zh-TW source, then English:
...
🏁 and the 🧭 authority-failure variant keep the headings already given in the numbered blocks below; they do not also take ❓ or ✋.

## The seven blocks — zh-TW literal set (source version)
...
### 5. 🗺️ 待拍板議題 — one line before each question
...
### 6. 🎯 收尾前最後候選議題 — when you think you have no more questions
...
### 7. 🏁 收尾檢核 — at the end of each round
...
### 6. 🎯 Final candidate questions before closing
...
- **Machine-facing text is always English**, regardless of the environment language: the supersession-scan sub-agent prompt, the verdict token it emits (`SCAN …: none`), and the reviewer input the documentation gate freezes for `/emergent-adr check-should-write-adr`. Verdict tokens never switch with the environment language — they are machine-parse targets, not human interface.
